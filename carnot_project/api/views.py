...
def ingest_id_and_latest_time_to_redis():
    ...
    for ids in distinct_ids:
        unique_row = CarnotTable.objects.filter(device_fk_id=ids.device_fk_id).order_by("-sts")[0]
...
def url_list(request):
    list = {
        "device_latest_detail_api":"http://127.0.0.1:8000/api/details/<int:pk>",
        "example of device_latest_detail_api":"http://127.0.0.1:8000/api/device_latest_detail_api/25029",
        "device_start_end_location":"http://127.0.0.1:8000/api/device_start_end_location/<int:pk>",
        "example of device_start_end_location":"http://127.0.0.1:8000/api/device_start_end_location/25029",
        "device_start_end_time_and_location":"http://127.0.0.1:8000/api/device_start_end_time_and_location/<int:pk>/<str:starttime>/<str:endtime>",
        "example of device_start_end_time_and_location":"http://127.0.0.1:8000/api/device_start_end_time_and_location/25029/2021-10-23T14:07:50.67886Z/2021-10-23T14:08:08.5984Z",
    }
...
def device_start_end_location(request, pk):
    if CarnotTable.objects.filter(device_fk_id=pk).exists():
        ...
        if cached_data is not None:
            ...
            result = {pk:{"start_location":start_location,"end_location":end_location}}
            cache.set(cache_key, json.dumps(result))
...
def device_start_end_time_and_location(request, pk, starttime, endtime):
    ...
    if CarnotTable.objects.filter(device_fk_id=pk).exists():
        ...
        data = CarnotTable.objects.filter(device_fk_id=pk, sts__gte=start_time, sts__lte=end_time)
        results = {}
        result_list = []
        for row in data:
